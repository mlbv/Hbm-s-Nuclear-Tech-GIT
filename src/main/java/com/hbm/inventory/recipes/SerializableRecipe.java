...
public abstract class SerializableRecipe {
    ...
    public static void initialize() {
        File recDir = new File(proxy.getDataDir().getPath() + "/config/hbm" + File.separatorChar + "hbmRecipes");
    ...
    public static void writeAStack(RecipesCommon.AStack astack, JsonWriter writer) throws IOException {
        ...
        if(astack instanceof RecipesCommon.ComparableStack) {
            ...
            writer.value(Item.REGISTRY.getNameForObject(comp.toStack().getItem()).toString());	//item name
    ...
    public static void writeItemStack(ItemStack stack, JsonWriter writer) throws IOException {
        writer.beginArray();
        writer.setIndent("");
        writer.value(Item.REGISTRY.getNameForObject(stack.getItem()).toString());						//item name
    ...
    public static void writeItemStackChance(Tuple.Pair<ItemStack, Float> stack, JsonWriter writer) throws IOException {
        writer.beginArray();
        writer.setIndent("");
        writer.value(Item.REGISTRY.getNameForObject(stack.getKey().getItem()).toString());											//item name
    ...
    public static boolean matchesIngredients(ItemStack[] inputs, RecipesCommon.AStack[] recipe) {
        ...
        for(RecipesCommon.AStack ingredient : recipe) recipeList.add(ingredient);
        ...
        for(int i = 0; i < inputs.length; i++) {
            ...
            if(inputStack != null) {
                ...
                Iterator<RecipesCommon.AStack> iterator = recipeList.iterator();

                while(iterator.hasNext()) {
                    RecipesCommon.AStack recipeStack = iterator.next();
